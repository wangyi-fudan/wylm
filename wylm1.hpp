...
#include	<time.h>
#include	<omp.h>
template<unsigned	input,	unsigned	hidden,	unsigned	depth,	unsigned	output,	unsigned	heads,	unsigned	batch>
class	wylm{
private:
	float	activate(float	x) {	return  x/sqrtf(1+x*x);	}
	float	gradient(float	x) {	x=1-x*x;	return	x*sqrtf(x);	}
	unsigned	size(void){	return	heads*(2*output*hidden+input*hidden)+hidden+(depth-1)*hidden*hidden+output*hidden;	}
	bool	drop(uint64_t	key,	uint64_t	l,	uint64_t	b,	uint64_t	i,	float	p){	return	wy2u01(wyhash64(key,(l<<56)|(b<<28)|i))>p;	}
public:
	float	idrop,	hdrop,	weight[heads*(2*output*hidden+input*hidden)+hidden+(depth-1)*hidden*hidden+output*hidden];
	uint64_t	seed;
	omp_lock_t	lock[heads*(2*output+input)+1+(depth-1)*hidden+output];

	wylm(){
		seed=wyhash64(time(NULL),0);
		unsigned	n=size();
		float	v=sqrtf(0.5);
		for(unsigned	i=0;	i<n;	i++)	weight[i]=(i<heads*(2*output*hidden+input)*hidden+hidden?v:1)*wy2gau(wyrand(&seed));
	...
	bool	load(const	char	*F){
		...
		if(fread(&n,4,1,f)!=1||n!=heads)	return	false;
	...
	float	train(uint8_t	*x[batch],	uint64_t	key,	float	eta){
		float	a[2*depth*batch*hidden+batch*output]={},wh=1/sqrtf(hidden),wi=1/sqrtf(input*heads),*w,*w0,*w1,*p,*q,*g,*h,*o;
		...
		#define woff(i,l)	(heads*(2*output*hidden+input*hidden)+hidden+((l)-1)*hidden*hidden+(i)*hidden)
		float	grad[size()]={};
		unsigned	b,	i,	j,	l,	c;
		for(b=0;	b<batch;	b++){
			p=aoff(b,0);
			for(c=0;	c<heads;	c++){
				for(i=0;	i<input-1;	i++)	if(drop(key^c,255,b,i,idrop)){
					w =weight+c*(2*output+input)*hidden+2*output*hidden+i*hidden;	
					w0=weight+c*(2*output+input)*hidden+x[b][i]*hidden;	
					w1=weight+c*(2*output+input)*hidden+(output+x[b][input-1])*hidden;
					for(j=0;	j<hidden;	j++)	p[j]+=w[j]*w0[j]*w1[j];
				}
			}
			w=weight+heads*(2*output*hidden+input*hidden);
		...
		for(b=0;	b<batch;	b++){
			p=aoff(b,0);	g=doff(b,0);	w=grad+heads*(2*output*hidden+input*hidden);
			...
			for(c=0;	c<heads;	c++){
				for(i=0;	i<input-1;	i++)	if(drop(key^c,255,b,i,idrop)){
					...
					w0=weight+c*(2*output+input)*hidden+x[b][i]*hidden;	
					w1=weight+c*(2*output+input)*hidden+(output+x[b][input-1])*hidden;
					p=grad+c*(2*output+input)*hidden+2*output*hidden+i*hidden;	
					q=grad+c*(2*output+input)*hidden+x[b][i]*hidden;	
					h=grad+c*(2*output+input)*hidden+(output+x[b][input-1])*hidden;
	...
	unsigned	sample(uint8_t	*x,	float	*o,	float	alpha){
		float	a[depth*hidden]={},wh=1/sqrtf(hidden),wi=1/sqrtf(input*heads),s,*w,*w0,*w1,*p,*q;
		unsigned	i,	j,	l,	c;
		for(c=0;	c<heads;	c++){
			for(i=0;	i<input-1;	i++){
				w =weight+c*(2*output+input)*hidden+2*output*hidden+i*hidden;	
				w0=weight+c*(2*output+input)*hidden+x[i]*hidden;	
				w1=weight+c*(2*output+input)*hidden+(output+x[input-1])*hidden;
		...
		for(l=1;	l<depth;	l++){
			...
			for(i=0;	i<hidden;	i++){
				s=0;	w=weight+heads*(2*output*hidden+input*hidden)+hidden+(l-1)*hidden*hidden+i*hidden;
		...
		for(i=0;	i<output;	i++){
			s=0;	w=weight+heads*(2*output*hidden+input*hidden)+hidden+(depth-1)*hidden*hidden+i*hidden;
