...
public:
	...
	wylm(){
		...
		float	v=cbrtf(0.5);
		for(unsigned	i=0;	i<wylm_size;	i++)	weight[i]=(i<2*output*hidden+input*hidden?v:1)*wy2gau(wyrand(&seed));
	...
	float	train(uint8_t	*x[batch],	uint64_t	key,	float	eta){
		...
		for(b=0;	b<batch;	b++){
			p=aoff(b,0);
			for(i=0;	i<input;	i++)	if(drop(key,255,b,i,idrop)){
		...
		for(size_t	i=0;	i<wylm_size;	i+=wylm_stride){
			p=weight+i;	q=grad+i;
			omp_set_lock(lock+i/wylm_stride);
			for(size_t	j=0;	j<wylm_stride;	j++)	p[j]-=q[j];
