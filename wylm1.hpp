...
public:
	...
	float	train(uint8_t	*x[batch],	uint64_t	key,	float	eta){
		...
		for(b=0;	b<batch;	b++){
			...
			for(i=0;	i<input-1;	i++)	if(drop(key,255,b,i,idrop)){
				w=weight+2*output*hidden+i*hidden;	w0=weight+x[b][i]*hidden;	w1=weight+output*hidden+x[b][input-1]*hidden;
		...
		for(b=0;	b<batch;	b++){
			...
			for(i=0;	i<input-1;	i++)	if(drop(key,255,b,i,idrop)){
				w=weight+2*output*hidden+i*hidden;	w0=weight+x[b][i]*hidden;	w1=weight+output*hidden+x[b][input-1]*hidden;
				p=grad+2*output*hidden+i*hidden;	q=grad+x[b][i]*hidden;	h=grad+output*hidden+x[b][input-1]*hidden;
	...
	unsigned	sample(uint8_t	*x,	float	*o,	float	alpha){
		...
		unsigned	i,	j,	l;
		for(i=0;	i<input-1;	i++){
			w=weight+2*output*hidden+i*hidden;	w0=weight+x[i]*hidden;	w1=weight+output*hidden+x[input-1]*hidden;
