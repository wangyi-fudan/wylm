...
private:
	...
	unsigned	size(void){	return	output*hidden+input*hidden+hidden+(depth-1)*hidden*hidden+output*hidden;	}
	bool	drop(uint64_t	key,	uint64_t	l,	uint64_t	b,	uint64_t	i){	return	wy2u01(wyhash64(key,(l<<56)|(b<<28)|i))>dropout;	}
public:
	float	*weight,	dropout;
	uint64_t	seed;
	...
	float	train(uint8_t	*x[batch],	uint64_t	key,	float	eta){
		float	a[2*depth*batch*hidden+batch*output]={},wh=1/sqrtf(hidden),wi=1/sqrtf(input+1),*w,*w1,*p,*q,*g,*o;
		...
		#define woff(i,l)	(output*hidden+input*hidden+hidden+((l)-1)*hidden*hidden+(i)*hidden)
		float	grad[size()]={};
		unsigned	b,	i,	j,	l;
		for(b=0;	b<batch;	b++){
			p=aoff(b,0);
			for(i=0;	i<input;	i++)	if(drop(key,255,b,i)){
				w=weight+x[b][i]*hidden;	w1=weight+output*hidden+i*hidden;
				for(j=0;	j<hidden;	j++)	p[j]+=w[j]*w1[j];
			}
			w=weight+output*hidden+input*hidden;
			for(i=0;	i<hidden;	i++){	p[i]=activate(wi*(p[i]+w[i]))*drop(key,0,b,i);	}	p[0]=1;
		}
		for(l=1;	l<depth;	l++){
			sgemm<1,0,hidden,batch,hidden,hidden,hidden,hidden,0>(wh,weight+woff(0,l),aoff(0,l-1),aoff(0,l));
			for(unsigned    b=0;    b<batch;    b++){
				p=aoff(b,l);
				for(unsigned	i=0;	i<hidden;	i++){	p[i]=activate(p[i])*drop(key,l,b,i);	}	p[0]=1;
			}
		}
		...
		for(l=depth-1;	l;	l--) {
			for(unsigned	b=0;	b<batch;	b++){
				...
				for(unsigned	i=0;	i<hidden;	i++)	q[i]*=gradient(p[i])*wh*drop(key,l,b,i);
			}
		...
		for(b=0;	b<batch;	b++){
			p=aoff(b,0);	g=doff(b,0);	w=weight+output*hidden+input*hidden;
			for(i=0;	i<hidden;	i++){	g[i]*=gradient(p[i])*wi*drop(key,0,b,i);	w[i]-=g[i];	}
			for(i=0;	i<input;	i++)	if(drop(key,255,b,i)){
				w=weight+x[b][i]*hidden;	w1=weight+output*hidden+i*hidden;
				p=grad+x[b][i]*hidden;	q=grad+output*hidden+i*hidden;
				for(j=0;	j<hidden;	j++){	p[j]+=g[j]*w1[j];	q[j]+=g[j]*w[j];	}
			}
		}
		const	size_t	n=size();
	...
	unsigned	sample(uint8_t	*x,	float	*o,	float	alpha){
		float	a[depth*hidden]={},wh=1/sqrtf(hidden),wi=1/sqrtf(input+1),s,*w,*w1,*p,*q;
		unsigned	i,	j,	l;
		for(i=0;	i<input;	i++){
			w=weight+x[i]*hidden;	w1=weight+output*hidden+i*hidden;
			for(j=0;	j<hidden;	j++)	a[j]+=w[j]*w1[j];
		}
		w=weight+output*hidden+input*hidden;
		for(i=0;	i<hidden;	i++){	a[i]=activate(wi*((1-dropout)*a[i]+w[i]))*(1-dropout);	}	a[0]=1;
		for(l=1;	l<depth;	l++){
			p=a+(l-1)*hidden;	q=p+hidden;
			for(i=0;	i<hidden;	i++){
				s=0;	w=weight+output*hidden+input*hidden+hidden+(l-1)*hidden*hidden+i*hidden;
				for(j=0;	j<hidden;	j++)	s+=p[j]*w[j];
				q[i]=s;
			}
			for(i=0;	i<hidden;	i++){	q[i]=activate(q[i]*wh)*(1-dropout);	}	q[0]=1;
		}
		p=a+(depth-1)*hidden;
		float	ma=-FLT_MAX,	sum=0;
		for(i=0;	i<output;	i++){
			s=0;	w=weight+output*hidden+input*hidden+hidden+(depth-1)*hidden*hidden+i*hidden;
